...
code) and [ASSIGNMENTS.md](ASSIGNMENTS.md) (26 exercises plus a capstone). Both link the
upstream DuckDB and Temporal documentation section by section — this repo is an
opinionated tour of those docs, not a replacement for them.
...
|---|---|---|
| [`QUALIFY`](https://duckdb.org/docs/stable/sql/query_syntax/qualify) | `silver_cleanse` | a subquery around every `row_number()` dedupe |
| [named `WINDOW`](https://duckdb.org/docs/stable/sql/functions/window_functions) | `silver_cleanse` | three `lag()` calls repeating the same frame |
| [`ASOF JOIN`](https://duckdb.org/2023/09/15/asof-joins-fuzzy-temporal-lookups.html) | `silver_enrich` | a correlated `max(valid_from)` subquery |
| `ASOF LEFT JOIN` | `silver_enrich` | silent row loss when weather is missing |
| [`GROUP BY ALL`](https://duckdb.org/docs/stable/sql/query_syntax/groupby) | both gold steps | the classic "added a dimension, forgot the GROUP BY" bug |
| [`* EXCLUDE (…)`](https://duckdb.org/docs/stable/sql/expressions/star) | `silver_enrich` | listing 14 columns to drop 3 |
...
| [`read_csv` / `read_parquet` / `COPY`](https://duckdb.org/docs/stable/data/parquet/overview) | everywhere | a loader |
| [`parquet_file_metadata`](https://duckdb.org/docs/stable/data/parquet/metadata) | `probe_inputs` | scanning a file to find out how big it is |
| `duckdb_memory()` | every step | guessing whether a step spilled |
| [`INSERT … BY NAME`](https://duckdb.org/docs/stable/sql/statements/insert) | `publish` | a positional insert that breaks on reorder |
| `range()` + `hash()` + list literals | the generator | numpy, pandas and faker |

Most of these are what DuckDB calls
[friendly SQL](https://duckdb.org/docs/stable/sql/dialect/friendly_sql), which
undersells them: `ASOF JOIN` and `RANGE` frames are different algorithms, not
shorter syntax.
...
tests/                52 tests. 45 need no infrastructure -- including 16 that
                      check the docs' own cross-references, because those drifted once
                      and prose cannot be type-checked any other way.
```
...
make test                      # 45 tests, no infrastructure
...
| If you want to… | Read |
|---|---|
| learn this properly, in order | **[ROADMAP.md](ROADMAP.md)** — six phases, with "done when" checks |
| understand a concept | [LEARN.md](LEARN.md) — and its [reading list](LEARN.md#reading-list) |
| actually practise | [ASSIGNMENTS.md](ASSIGNMENTS.md) — 26 exercises and a capstone |
| change the pipeline | `src/duckflow/steps.py` — it is all there |

The three most useful pages upstream, if you read nothing else:
[DuckDB concurrency](https://duckdb.org/docs/stable/connect/concurrency) (the
specification behind the folklore),
[Temporal event history](https://docs.temporal.io/encyclopedia/event-history)
(replay, from the people who built it), and
[Big Data is Dead](https://motherduck.com/blog/big-data-is-dead/) (why any of
this is worth trying on one box).
